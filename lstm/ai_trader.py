...
class Trading:
    ...
    def transaction(self, target):
        ...
        for episode in range(1, episodes + 1):
            ...
            total_profit = 0
            trader.inventory = []
            for t in tqdm(range(data_sample)):
                action = trader.trade(state)
                next_state = self.state_creator(data, t + 1,window_size + 1)
                reward = 0
                if action ==1: #Buying
                    trader.inventory.append(data[t])
                    print('트레이더가 매수함 :',self.stock_price_format(data[t]))
                elif action ==2 and len(trader.inventory) >0: # Selling
                    buy_price = trader.inventory.pop(0)
                    reward = max(data[t] - buy_price,0)
                    total_profit +=data[t] - buy_price
                    print('트레이더가 매도:',self.stock_price_format(data[t]),', 이익: '+self.stock_price_format(data[t] - buy_price))
                if t == data_sample -1:
                    done = True
                else:
                    done = False
